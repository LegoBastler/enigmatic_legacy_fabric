...
public class SuperpositionHandler {
    ...
    // These things below probably have the wrong value
    ...
    // public static boolean hasAdvancedTrinkets(final LivingEntity entity) {
    //     return SuperpositionHandler.getAdvancedTrinkets(entity).size() > 0;
    // }

    // public static boolean unlockSpecialSlot(String slot, PlayerEntity player) {
    ...
    // public static List<ItemStack> getAdvancedTrinkets(final LivingEntity entity) {
    ...
    public static boolean hasTrinket(final LivingEntity entity, final Item trinket) {
        final Optional<ImmutableTriple<String, Integer, ItemStack>> data = CuriosApi.getCuriosHelper().findEquippedCurio(trinket, entity);
     ...
     */
    @Nullable
    public static ItemStack getCurioStack(final LivingEntity entity, final Item trinket) {
        final Optional<ImmutableTriple<String, Integer, ItemStack>> data = CuriosApi.getCuriosHelper().findEquippedCurio(trinket, entity);
        if (data.isPresent())
    ...
    public static boolean tryForceEquip(LivingEntity entity, ItemStack trinket) {
        ...
        TrinketItem item = (TrinketItem) trinket.getItem();
            ...
            if (!entity.getWorld().isClient()) {
                ...
                cycle: for (Map.Entry<String, ICurioStacksHandler> entry : curios.entrySet()) {
                    ...
                    for (int i = 0; i < stackHandler.getSlots(); i++) {
                        ...
                        Set<String> tags = CuriosApi.getCuriosHelper().getCurioTags(trinket.getItem());
                        String id = entry.getKey();

                        SlotContext context = new SlotContext(id, entity, i, false, entry.getValue().isVisible());

                        if (present.isEmpty() && (tags.contains(id) || tags.contains("trinket")) && item.canEquip(context, trinket)) {
                            stackHandler.setStackInSlot(i, trinket);
                            item.playRightClickEquipSound(entity, trinket);
                            equipped.setTrue();
                            break cycle;
                        }
                    }
     ...
     * @param slots      How many slots of this trinket will be available by default.
     * @param isEnabled  Whether or not this trinket type should be initially
     *                   acessible by player.
     * @param isHidden   Whether or not this trinket type should be hidden from
     *                   default Curio GUI.
     * @param icon       Optional resource location for custom icon.
     */
    public static void registerCurioType(final String identifier, final int slots, final boolean isHidden, final ResourceLocation icon) {
    ...
    public static Box getBoundingBoxAroundEntity(final LivingEntity entity, final double radius) {
    ...
    public static void setEntityMotionFromVector(final Entity entity, final Vec3d originalPosVector, final float modifier) {
        final Vec3d entityVector = entity.getPos();
    ...
    public static List<LivingEntity> getObservedEntities(PlayerEntity player, World world, float range, int maxDist, boolean stopWhenFound) {
        Vec3d target = player.getPos();
        List<LivingEntity> entities = new ArrayList<>();

        for (int distance = 1; distance < maxDist; ++distance) {
            target = target.add(new Vec3d(player.getRotationVec()).multiply(distance)).add(0.0, 0.5, 0.0);
            List<LivingEntity> list = player.getWorld().getEntitiesByClass(LivingEntity.class,
                        new Box(target.x - range, target.y - range, target.z - range, target.x + range, target.y + range, target.z + range),
                        LivingEntity -> false);
            list.removeIf(entity -> entity == player || !player.canSee(entity));
    ...
    public static boolean doesObserveEntity(PlayerEntity player, LivingEntity entity) {
        Vec3d vector3d = player.getRotationVector().normalize();
        Vec3d vector3d1 = new Vec3d(entity.getX() - player.getX(), entity.getEyeY() - player.getEyeY(), entity.getZ() - player.getZ());
        double d0 = vector3d1.length();
        vector3d1 = vector3d1.normalize();
        double d1 = vector3d.dotProduct(vector3d1);

        return d1 > 1.0D - 0.025D / d0 && player.canSee(entity);
    }
    ...
    public static void lookAt(double px, double py, double pz, PlayerEntity me) {
        ...
        yaw += 90f;
        me.setAngles((float) yaw, (float) pitch);
    ...
    public static boolean validTeleport(LivingEntity entity, double x_init, double y_init, double z_init, World world, int checkAxis) {
        int x = (int) x_init;
        int y = (int) y_init;
        int z = (int) z_init;

        BlockState block = world.getBlockState(new BlockPos(x, y - 1, z));
        if (!world.isSpaceEmpty(new Box(new BlockPos(x, y - 1, z))) && block.isOpaque()) {
            for (int counter = 0; counter <= checkAxis; counter++) {
                if (!world.isSpaceEmpty(new Box(new BlockPos(x, y + counter - 1, z))) && world.getBlockState(new BlockPos(x, y + counter - 1, z)).isOpaque() && world.isSpaceEmpty(new Box(new BlockPos(x, y + counter, z))) && world.isSpaceEmpty(new Box(new BlockPos(x, y + counter + 1, z)))) {
                    world.playSound(null, entity.getBlockPos(), SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1.0F, (float) (0.8F + (Math.random() * 0.2D)));
                    ...
                    if (entity instanceof ServerPlayerEntity) {
                        ...
                        player.teleport(x + 0.5, y + counter, z + 0.5, false);
                    } else {
                        ((LivingEntity) entity).teleport(x + 0.5, y + counter, z + 0.5, false);
                    }
     ...
     */
    public static boolean validTeleportRandomly(LivingEntity entity, World world, int radius) {
        int d = radius * 2;
    ...
    public static Tag getPersistentTag(PlayerEntity player, String tag, Tag expectedValue) {
        CompoundTag data = player.getPersistentData();
        CompoundTag persistent;

        if (!data.contains(PlayerEntity.PERSISTED_NBT_TAG)) {
            data.put(PlayerEntity.PERSISTED_NBT_TAG, (persistent = new CompoundTag()));
        } else {
    ...
    public static void removePersistentTag(PlayerEntity player, String tag) {
        CompoundTag data = player.getPersistentData();
        CompoundTag persistent;
    ...
    public static void setPersistentTag(PlayerEntity player, String tag, Tag value) {
        CompoundTag data = player.getPersistentData();
        CompoundTag persistent;
    ...
    public static boolean doesAdvancementExist(ResourceLocation location) {
    ...
    public static void grantAdvancement(ServerPlayerEntity player, ResourceLocation location) {
    ...
    public static void revokeAdvancement(ServerPlayerEntity player, ResourceLocation location) {
        Advancement adv = player.server.getAdvancements().getAdvancement(location);
    ...
    public static void addPotionTooltip(List<MobEffectInstance> list, ItemStack itemIn, List<Text> lores, float durationFactor) {
        List<Pair<Attribute, AttributeModifier>> list1 = Lists.newArrayList();
        if (list.isEmpty()) {
            lores.add((Text.translatable("effect.none")).withStyle(Formatting.GRAY));
        } else {
            for (MobEffectInstance effectinstance : list) {
                MutableText iformattabletextcomponent = Text.translatable(effectinstance.getDescriptionId());
                ...
                if (effectinstance.getAmplifier() > 0) {
                    iformattabletextcomponent.append(" ").append(Text.translatable("potion.potency." + effectinstance.getAmplifier()));
                }
                ...
                lores.add(iformattabletextcomponent.formatted(effect.getCategory().getTooltipFormatting()));
            }
        }
        ...
        if (!list1.isEmpty()) {
            ...
            for (Pair<Attribute, AttributeModifier> pair : list1) {
                ...
                if (d0 > 0.0D) {
                    lores.add((Text.translatable("attribute.modifier.plus." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Text.translatable(pair.getFirst().getDescriptionId()))).withStyle(Formatting.BLUE));
                } else if (d0 < 0.0D) {
                    d1 = d1 * -1.0D;
                    lores.add((Text.translatable("attribute.modifier.take." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Text.translatable(pair.getFirst().getDescriptionId()))).withStyle(Formatting.DARK_RED));
                }
            }
        }
    ...
    public static boolean canPickStack(PlayerEntity player, ItemStack stack) {
        if (player.getInventory().getEmptySlot() >= 0)
            ...
            for (ItemStack invStack : allInventories) {
                if (SuperpositionHandler.canMergeStacks(invStack, stack, player.getInventory().getMaxCountPerStack()))
                    return true;
            }
        }
    ...
    public static boolean canMergeStacks(ItemStack stack1, ItemStack stack2, int invStackLimit) {
        return !stack1.isEmpty() && SuperpositionHandler.stackEqualExact(stack1, stack2) && stack1.isStackable() && stack1.getCount() < stack1.getMaxCount() && stack1.getCount() < invStackLimit;
    }
    ...
    public static boolean isPermanentlyDead(PlayerEntity player) {
    ...
    //     ServerChunkCache cache = (ServerChunkCache) player.getWorld().getChunkSource();
    //
    ...
    public static boolean hasExactStack(PlayerEntity player, ItemStack stack) {
        for (List<ItemStack> list : player.getInventory().compartments) {
    ...
    public static boolean ifDroplistContainsItem(Collection<ItemEntity> drops, Item item) {
        ...
        for (ItemEntity drop : drops) {
            if (drop.getStack() != null)
                if (drop.getStack().getItem() == item)
                    return true;
        }
    ...
    public static boolean isAffectedBySoulLoss(PlayerEntity player, boolean hadRing) {
    ...
    public static boolean canDropSoulCrystal(PlayerEntity player, boolean hadRing) {
    ...
    public static void sendToDimension(ServerPlayerEntity player, RegistryKey<World> dimension, ITeleporter teleporter) {
        if (!player.getWorld().getRegistryKey().equals(dimension)) {
            ServerWorld world = SuperpositionHandler.getWorld(dimension);
            if (world != null) {
    ...
    public static void sendToDimension(ServerPlayerEntity player, RegistryKey<World> dimension) {
    ...
    public static ServerWorld backToSpawn(ServerPlayerEntity serverPlayer) {
        ...
        serverPlayer.getWorld().playSound(null, serverPlayer.getBlockPos(), SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1.0F, (float) (0.8F + (Math.random() * 0.2)));
        ...
        Optional<Vec3d> vec = AdvancedSpawnLocationHelper.getValidSpawn(respawnWorld, serverPlayer);
        Optional<Vec3d> vec2;
        ServerWorld destinationWorld = vec.isPresent() ? respawnWorld : serverPlayer.server.getOverworld();

        if (!serverPlayer.getWorld().equals(destinationWorld)) {
    ...
    public static DimensionalPosition getRespawnPoint(ServerPlayerEntity serverPlayer) {
        ...
        Optional<Vec3d> currentDimensionRespawnCoords = AdvancedSpawnLocationHelper.getValidSpawn(respawnWorld, serverPlayer);
        Optional<Vec3d> destinationDimensionRespawnCoords;
        ...
        if (currentDimensionRespawnCoords.isPresent()) {
             ...
             */
            trueVec = new Vec3d(destinationWorld.getSpawnPos().getX() + 0.5, destinationWorld.getSpawnPos().getY() + 0.5, destinationWorld.getSpawnPos().getZ() + 0.5);
    ...
    public static boolean hasArchitectsFavor(PlayerEntity player) {
    ...
    public static String getSufferingTime(PlayerEntity player) {
        if (player == null)
            return "0%";
        else {
            var counter = IPlaytimeCounter.get(player);
    ...
    public static List<ItemStack> getFullEquipment(PlayerEntity player) {
        ...
        equipmentStacks.add(player.getMainHandStack());
        equipmentStacks.add(player.getOffhandStack());
    ...
    public static boolean areWeDedicatedServer() {
        return FabricLoader.getInstance().getEnvironmentType() == EnvType.SERVER && FabricLoader.getInstance().getEnvironmentType() != EnvType.CLIENT;
    }
    ...
    public static boolean hasAntiInsectAcknowledgement(PlayerEntity player) {
        List<ItemStack> heldItems = Lists.newArrayList(player.getMainHandStack(), player.getOffHandStack());

        for (ItemStack held : heldItems) {
            if (held != null && held.getItem() instanceof TheAcknowledgment) {
                if (EnchantmentHelper.getLevel((RegistryEntry<Enchantment>) Enchantments.BANE_OF_ARTHROPODS, held) > 0)
                    return true;
            }
        }
    ...
    public static boolean isStaringAt(PlayerEntity player, LivingEntity living) {
        Vec3d vector3d = player.getRotationVector().normalize();
        Vec3d vector3d1 = new Vec3d(living.getX() - player.getX(), living.getEyeY() - player.getEyeY(), living.getZ() - player.getZ());
        double d0 = vector3d1.length();
        vector3d1 = vector3d1.normalize();
        double d1 = vector3d.dotProduct(vector3d1)
    ...
    public static Optional<Tuple<UUID, BlockPos>> updateSoulCompass(ServerPlayerEntity player) {
        var optional = SoulArchive.getInstance().findNearest(player.getWorld(), player.getBlockPos());
    ...
    public static ItemStack mergeEnchantments(ItemStack input, ItemStack mergeFrom, boolean overmerge, boolean onlyTreasure) {
        ...
        ItemEnchantmentsComponent inputEnchants = EnchantmentHelper.getEnchantments(returnedStack);
        ItemEnchantmentsComponent mergedEnchants = EnchantmentHelper.getEnchantments(mergeFrom);
    ...
    public static ItemStack getEnigmaticElytra(LivingEntity living) {
        ItemStack stack = living.getEquippedStack(EquipmentSlot.CHEST);

        if (stack.isOf(EnigmaticItems.ENIGMATIC_ELYTRA))
            return stack;
        else
    ...
    public static void onDamageSourceBlocking(LivingEntity blocker, ItemStack useItem, DamageSource source, CallbackInfoReturnable<Boolean> info) {
        if (blocker instanceof PlayerEntity player && useItem != null) {
            ...
            if (blocking && useItem.getItem() instanceof InfernalShield) {
                ...
                if (!source.is(DamageTypeTags.BYPASSES_SHIELD) && ((IProperShieldUser) blocker).isActuallyReallyBlocking() && !piercingArrow) {
                    Vec3d sourcePos = source.getSourcePosition();
                    if (sourcePos != null) {
                        Vec3d lookVec = blocker.getViewVector(1.0F);
                        Vec3d sourceToSelf = sourcePos.vectorTo(blocker.position()).normalize();
    ...
    @OnlyIn(Dist.CLIENT)
    public static void obscureTooltip(List<Text> tooltip) {
        tooltip.replaceAll(text -> Text.literal(obscureString(text.getString()))
                .formatted(text.getStyle()));
    }
    ...
    @OnlyIn(Dist.CLIENT)
    public static LevelInfo getAltInfo(LevelSummary summary) {
        LevelInfo info = summary.getLevelInfo()
        File world = getSaveFolder(summary);
        boolean fractured = isWorldFractured(world);

        if (summary.getGameMode() == GameMode.SURVIVAL || fractured) {
            ...
            MutableText tcn = Text.translatable(key);
            tcn.formatted(info.getStyle());

            if (info instanceof MutableText mutable)
